{-
  ...
  MonoRef.Dynamics.Store.Evolving.Store exports various tools to work stores
...
module MonoRef.Dynamics.Store.Evolving.Store
  (_⟹_ : Type → Type → Set)
  (Inert : ∀ {A B} → A ⟹ B → Set)
  where
...
module ParamStore
  ...
  update-store : ∀ {Σ A} → A ∈ Σ → EvolvingStoreValue A Σ → Store Σ → Store Σ
  update-store ptr v μ = μ All[ ptr ]≔' fromDelayedCast v

  store-lookup-v : ∀ {Σ A} → A ∈ Σ → Store Σ → Σ ∣ ∅ ⊢ A
  store-lookup-v A∈Σ ν with lookup-store A∈Σ ν
  ... | fromDelayedCast (intro {e = e} _ _) = e
  ...
  μ-static-update R x μ v =
    update-store (ref⟹∈ R) (subst (λ x → x) (helper R x) (intro (v⇑ v) (Type⇑ _))) μ
     where
       helper : ∀ {Σ Γ B} {r : Σ ∣ Γ ⊢ Ref B}
         → (R : SimpleValue r)
         → static B
         → EvolvingStoreValue B Σ ≡ EvolvingStoreValue (ref⟹T R) Σ
  ...
  μ-update : ∀ {Σ A}
    ...
    → Value e
    → Store Σ
  μ-update A∈Σ ν v = update-store A∈Σ (intro (v⇑ v) (Type⇑ _)) ν
