...
-- ‖ fail         ‖ʰᶠ = 1
-- ‖ (injSeq B g) ‖ʰᶠ = (1 + ‖ B ‖ʰᵢₜ) ⊔ ‖ g ‖ʰₘ
-- ‖ (middle g)   ‖ʰᶠ = ‖ g ‖ʰₘ

-- ‖ (prjSeq A i) ‖ʰ = (1 + ‖ A ‖ʰᵢₜ) ⊔ ‖ i ‖ʰᶠ
-- ‖ (final i)    ‖ʰ = ‖ i ‖ʰᶠ


-- mk-nfcoercion-height : ∀ A B
--   → ‖ (make-normal-form-coercion A B) ‖ʰ ≤ suc (‖ A ‖ʰₜ ⊔ ‖ B ‖ʰₜ)
-- mk-nfcoercion-height A B with ⌣-decidable A B
-- ... | no _ = m≤m+n 1 _
-- mk-nfcoercion-height .`ℕ .`ℕ | yes ⌣-ℕ-refl = s≤s z≤n
-- mk-nfcoercion-height .Unit .Unit | yes ⌣-Unit-refl = s≤s z≤n
...
--     suc (‖ c ‖ʰ ⊔ ‖ d ‖ʰ)
--       ≤⟨ +-monoʳ-≤ 1 (⊔-mono-≤ l r) ⟩
--     suc (suc (‖ A' ‖ʰₜ ⊔ ‖ A ‖ʰₜ) ⊔ suc (‖ B ‖ʰₜ ⊔ ‖ B' ‖ʰₜ))
--       ≤⟨ s≤s (1+b⊔a⊔c⊔d≤1+a⊔c⊔b⊔d{‖ A ‖ʰₜ}{‖ A' ‖ʰₜ}) ⟩
--     suc (suc (‖ A ‖ʰₜ ⊔ ‖ B ‖ʰₜ ⊔ (‖ A' ‖ʰₜ ⊔ ‖ B' ‖ʰₜ)))
--   ∎
-- mk-nfcoercion-height (A `× B) (A' `× B') | yes ⌣-×
--   with make-normal-form-coercion A A' | mk-nfcoercion-height A A'
--      | make-normal-form-coercion B B' | mk-nfcoercion-height B B'
...
--       ≤⟨ +-monoʳ-≤ 1 (⊔-mono-≤ l r) ⟩
--     suc (suc (‖ A ‖ʰₜ ⊔ ‖ A' ‖ʰₜ) ⊔ suc (‖ B ‖ʰₜ ⊔ ‖ B' ‖ʰₜ))
--       ≤⟨ s≤s (1+a⊔b⊔c⊔d≤1+a⊔c⊔b⊔d{‖ A ‖ʰₜ}{‖ A' ‖ʰₜ}) ⟩
--     suc (suc (‖ A ‖ʰₜ ⊔ ‖ B ‖ʰₜ ⊔ (‖ A' ‖ʰₜ ⊔ ‖ B' ‖ʰₜ)))
--   ∎
-- mk-nfcoercion-height .(Ref _) (Ref B) | yes ⌣-ref = {!!}

-- mk-fcoercion-height : ∀ {A B} → (A≢⋆ : Injectable A) → (B≢⋆ : Injectable B)
--   → ‖ (make-final-coercion A≢⋆ B≢⋆) ‖ʰᶠ ≤ suc (‖ A≢⋆ ‖ʰᵢₜ ⊔ ‖ B≢⋆ ‖ʰᵢₜ)
-- mk-fcoercion-height A≢⋆ B≢⋆ with ⌣-decidableᵢ A≢⋆ B≢⋆
-- ... | no _ = m≤m+n 1 _
-- ... | yes ⌣-ℕ-refl = s≤s z≤n
-- ... | yes ⌣-Unit-refl = s≤s z≤n
-- mk-fcoercion-height () _ | yes ⌣-⋆L
...
--     suc (suc (‖ A ‖ʰₜ ⊔ ‖ B ‖ʰₜ ⊔ (‖ A' ‖ʰₜ ⊔ ‖ B' ‖ʰₜ)))
--   ∎
-- mk-fcoercion-height {Ref A} {Ref B} _ _ | yes ⌣-ref = {!!}
